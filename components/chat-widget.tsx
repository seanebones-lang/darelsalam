...
export function ChatWidget({ disclaimer, suggestedPromptsRaw }: ChatWidgetProps) {
  const { t } = useLocale();
  const suggestions = parsePrompts(suggestedPromptsRaw);
  ...
  async function submit(text: string) {
    ...
    try {
      ...
      if (!response.ok || !payload.reply) {
        throw new Error(payload.error ?? "Unable to reach assistant");
      }
      setMessages((hist) => [...hist, { role: "assistant", content: payload.reply }]);
    } catch (e) {
  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={setOpen}>
      ...
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content className="fixed z-50 flex w-[min(calc(100vw-1.5rem),400px)] flex-col rounded-[28px] border border-emerald-900/20 bg-white p-4 shadow-[0_35px_80px_rgba(5,61,53,0.35)] outline-none sm:p-6 bottom-[max(5.5rem,calc(env(safe-area-inset-bottom,0px)+4.5rem))] end-[max(0.75rem,env(safe-area-inset-right,0px))] top-[max(1rem,env(safe-area-inset-top,0px))] max-h-[min(560px,calc(100dvh-2rem))] md:bottom-8 md:end-4 md:top-auto md:h-[560px] md:max-h-[560px] dark:border-emerald-500/35 dark:bg-emerald-900">
          <header className="mb-4 flex shrink-0 items-start justify-between gap-3">
            <div>
              ...
              <DialogPrimitive.Description className="mt-2 text-xs leading-relaxed text-zinc-600 dark:text-emerald-200">
                {disclaimer ?? t("chat.disclaimerDefault")}
          ...
          <div className="relative flex min-h-0 flex-1 flex-col gap-3 overflow-hidden rounded-2xl border border-emerald-900/15 bg-emerald-50/50 p-3 dark:border-emerald-600/35 dark:bg-emerald-950">
